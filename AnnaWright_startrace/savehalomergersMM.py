"""
savehalomergersMM.py

Track and save merger history particle data for stellar halos in Romulus zoom simulations.

This script traces star and dark matter particles from merger progenitor halos
across all simulation snapshots in Romulus zoom simulations. It identifies a 
specific merger event (by snapshot and halo number) and extracts the full time 
evolution of particles that originated in that merger progenitor, relative to 
the main host halo.
...
- Adapted for Romulus zoom simulation file structure

Author: Nithun Selva
Date: 2025

Usage:
    # Track merger 0384_47 for r442 simulation (main halo auto-detected)
    python savehalomergersMM.py r442 0384_47
    
    # Track multiple mergers for r329 simulation
    python savehalomergersMM.py r329 0291_1 0384_47 0512_3
    
    # Show help
    python savehalomergersMM.py --help
...
if 'emu' in hostname:
    # EMU system paths for Romulus zooms
...
def get_halo(snapshot, halo_number, ss_dir):
    ...
    Args:
        ...
        ss_dir (str): Simulation directory name (e.g., 'r442.romulus25.3072g1HsbBH')
...
def setup_paths_and_data(simname):
    ...
    Args:
        simname (str): Name of Romulus zoom simulation (e.g., 'r442', 'r329', 'r377')
    ...
    # If desired, one could read in Elaad's halo trace output here to get the main halo's progenitors instead of using Tangos' built-in method.
    # The command would be 
    #     trace = pd.read_hdf(ss_z0 + '/' + basename + '.004096' + 'trace_back.hdf5')
    #     halo_snapshots_dict = {'004096': 1}
    #.    for ts in df.iloc[1].keys(): halo_snapshots_dict.update({ts: df.iloc[0][ts]})
...
def main(idx, simname):
    ...
    (simpath, outfile_dir, annafile_dir, basename, ss_dir, sim_base, ss_z0, 
     halo_particle_dict, main_halo, halo_snapshots_dict, hostids, partids, pct) = setup_paths_and_data(simname)
    ...
    timesteps_to_process = timesteps_to_process[len(timesteps_to_process)-len(halo_snapshots_dict):]  # Only process snapshots where the main halo exists
    ...
    for i, tstep in enumerate(tqdm.tqdm(timesteps_to_process, desc=f"Processing {simname} snapshots")):
        ...
        # Romulus zoom snapshots are in subdirectories
        snap_path = tstep.filename
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Track and save merger history particle data for stellar halos in Romulus zoom simulations.",
        epilog="""
Examples:
  # Track merger 0384_47 for r442 simulation
  python savehalomergersMM.py r442 0384_47
  
  # Track multiple mergers for r329 simulation
  python savehalomergersMM.py r329 0291_1 0384_47 0512_3
  
  # Track merger in r377 simulation
  python savehalomergersMM.py r377 1280_17
...
Note:
  This script automatically detects the main halo (first halo with stars at z=0).
  Unlike savehalomergers.py, it doesn't accept a halo number argument.
        """,
    ...
    parser.add_argument(
        'simname',
        help="Romulus zoom simulation name (e.g., r442, r329, r377)"
    )
    parser.add_argument(
        'merger_ids',
        nargs='+',
    ...
    if len(args.merger_ids) == 1:
        ...
        print(f"Processing merger {merger_id} in {simname} simulation")
        main(merger_id, simname)
    else:
